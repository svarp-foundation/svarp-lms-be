...
async def get_secure_media(
    filename: str,
    request: Request,
    token: str = Query(None, description="JWT Bearer Token required for media access"),
    ...
    Public QR codes for certificate verification are served directly without token.
    """
    # Security: Prevent directory traversal attacks
    if ".." in filename or filename.startswith("/"):
        raise HTTPException(status_code=400, detail="Invalid filename format")

    # Public QR code endpoint for verified certificates
    ...
    if not auth_token:
        raise HTTPException(status_code=403, detail="Bearer token required for media access")

    try:
        user = await auth.get_current_user(token=auth_token, db=db)
    except Exception:
        raise HTTPException(status_code=403, detail="Invalid or expired token for media access")

    if filename.startswith("SVARP-") and filename.endswith(".pdf"):
        cert_code = filename[:-4]
        cert = db.query(models.Certificate).filter(models.Certificate.certificate_code == cert_code).first()
        if cert:
            ...
            if course and student:
                ...
                signatory_name = "Mr. Vikash Kumar"
                if course.instructor and course.instructor.full_name and course.instructor.full_name.strip():
                    signatory_name = course.instructor.full_name.strip()
                elif getattr(course, "instructor_id", None):
                    instructor_user = db.query(models.User).filter(models.User.id == course.instructor_id).first()
                    if instructor_user and instructor_user.full_name and instructor_user.full_name.strip():
                        signatory_name = instructor_user.full_name.strip()
